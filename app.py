...
def fiscal_args():
    return request.args.get("fiscal_year", type=int), request.args.get("fiscal_month", type=int)
...
def validate_period(fiscal_year, fiscal_month):
    ...
    if fiscal_month is None or int(fiscal_month) < 1 or int(fiscal_month) > 13:
...
def get_customer(customer_id):
    return supabase.table("customers").select("*").eq("id", customer_id).single().execute().data
...
def rebuild_summary(fiscal_year, fiscal_month):
    ...
    payload = {
        ...
        "negative_readings": analysis["negative"],
        "exaggerated_readings": analysis["exaggerated"],
        "zero_readings": analysis["zero"],
        "disconnected_connections": analysis["disconnected"],
        "analysis": analysis,
        "updated_at": datetime.utcnow().isoformat(),
    }

    existing = (
        supabase.table("reading_period_summary")
        .select("id")
        .eq("fiscal_year", fiscal_year)
        .eq("fiscal_month", fiscal_month)
        .execute()
...
def generate_bill(reading):
    ...
    existing = (
        ...
        .eq("meter_reading_id", reading["id"])
        .execute()
        .data
    )
...
def dashboard():
    ...
    customer_count = supabase.table("customers").select("id", count="exact").execute().count or 0
    ...
    trend_rows = (
        supabase.table("billing")
        .select("fiscal_year,fiscal_month,consumption")
        .order("fiscal_year")
        .order("fiscal_month")
        .execute()
        .data
    ...
    return ok({
        ...
        "total_consumption": round(sum(float(r.get("consumption") or 0) for r in readings), 2),
        "total_bill_amount": round(sum(float(b.get("total_amount") or 0) for b in bills), 2),
        "pending_maintenance": maintenance_count,
        "active_meter_readers": reader_count,
        "analysis": analysis,
        "trend": trend,
    })
...
CUSTOMER_FIELDS = {
    "customer_code", "old_code", "name", "phone", "email", "address",
    "zone_id", "reading_group_id", "device_id", "direct_register", "status"
}
...
def customers_list():
    ...
    if q:
        req = req.or_(f"name.ilike.%{q}%,customer_code.ilike.%{q}%,old_code.ilike.%{q}%,phone.ilike.%{q}%")
    if status:
...
def customers_detail(customer_id):
    customer = (
        ...
        .select("*, zones(name), reading_groups(name)")
        .eq("id", customer_id)
        .single()
        .execute()
        .data
    ...
    return ok({"customer": customer, "readings": readings, "billing": billing})
...
def readings_create():
    ...
    if not payload.get("customer_id"):
        return fail("customer_id is required.")
...
def maintenance_list():
    rows = (
        supabase.table("maintenance_requests")
        .select("*, customers(name), employees(name)")
